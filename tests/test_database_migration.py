...
from app.database.session import (
    ...
    ensure_project_showrunner_json_column,
)
...
def test_existing_sqlite_project_table_gets_showrunner_json_column(tmp_path) -> None:
    engine = create_engine(f"sqlite:///{tmp_path / 'legacy_showrunner.db'}")
    with engine.begin() as connection:
        connection.execute(
            text(
                "CREATE TABLE projects ("
    ...
    columns = {column["name"] for column in inspect(engine).get_columns("projects")}
    assert "showrunner_json" in columns
    engine.dispose()
